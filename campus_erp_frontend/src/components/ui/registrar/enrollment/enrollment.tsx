  ...
  return (
    <Tabs defaultValue="account" orientation="vertical" className="flex-row items-stretch bg-white p-4 rounded-lg shadow-md w-full h-[85vh] border border-black/20">
      <TabsList className="grid w-70 grid-cols-1 gap-5 border-0 bg-white mr-7 h-[50vh]! shrink-0">
            <TabsTrigger value="admission-requirement" className="w-full gap-2 p-3 border-black/20 data-active:bg-primary data-active:text-primary-foreground">
                <AppWindowIcon />
                Admission Requirement
            </TabsTrigger>
            <TabsTrigger value="pre-enrollment" className="w-full gap-2 p-3 border-black/20 data-active:bg-primary data-active:text-primary-foreground">
                Pre-Enrollment
            </TabsTrigger>
            <TabsTrigger value="enrollment" className="w-full gap-2 p-3 border-black/20 data-active:bg-primary data-active:text-primary-foreground">
                Pre-Registration Record
            </TabsTrigger>
            <TabsTrigger value="grades" className="w-full gap-2 p-3 border-black/20 data-active:bg-primary data-active:text-primary-foreground">
                Transferee Evaluation
            </TabsTrigger>
            <TabsTrigger value="classes" className="w-full gap-2 p-3 border-black/20 data-active:bg-primary data-active:text-primary-foreground">
                Add / Remove Subjects
            </TabsTrigger>
            <TabsTrigger value="faculty-schedule" className="w-full gap-2 p-3 border-black/20 data-active:bg-primary data-active:text-primary-foreground">
